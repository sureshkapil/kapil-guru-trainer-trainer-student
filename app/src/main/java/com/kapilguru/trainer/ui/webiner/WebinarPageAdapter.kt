...
class WebinarPageAdapter(fragmentManager: FragmentManager, lifecycle: Lifecycle) : FragmentStateAdapter(fragmentManager, lifecycle) {
    var titles = arrayListOf("Live/Up coming", "Total Active")
    var tabSubTitles = arrayListOf("Webinars", "Webinars")

    override fun getItemCount(): Int = 2

    override fun createFragment(position: Int): Fragment {
        return when (position) {
            0 -> LiveUpComingWebinarsFragment()
            else -> ActiveWebinarsFragment()
        }
    }
