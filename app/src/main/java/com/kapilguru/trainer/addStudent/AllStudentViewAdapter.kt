...
class AllStudentViewAdapter(fragmentManager: FragmentManager, lifecycle: Lifecycle) :
    ...
    var titles = arrayListOf("Online", "Offline", "SignedUp")
    var tabSubTitles = arrayListOf("Students", "Students", "Students")

    override fun getItemCount(): Int = titles.size

    override fun createFragment(position: Int): Fragment {
        return when (position) {
            0 -> LiveAllMyStudentFragment()
            1 -> MyStudentsOfflineFragment()
            else  -> SignedUpStudentsFragments()
        }
    }
