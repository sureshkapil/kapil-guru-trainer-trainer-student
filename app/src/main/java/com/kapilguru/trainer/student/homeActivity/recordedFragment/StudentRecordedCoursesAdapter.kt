...
class StudentRecordedCoursesAdapter(var onItemClick: CardItem, var isFrom: Boolean) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class Holder(private val studentRecordedCoursesItemBinding: StudentRecordedCoursesItemBinding) : RecyclerView.ViewHolder(studentRecordedCoursesItemBinding.root) {
        var view = studentRecordedCoursesItemBinding
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == 0) {
            val view = StudentRecordedCoursesItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            return Holder(view)
        } else {
